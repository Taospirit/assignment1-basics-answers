...
class BPE_Tokenizer:
    ...
    def from_files(cls, vocab_filepath, merges_filepath, special_tokens=None):
        ...
        load_file = lambda filepath: pickle.load(open(filepath, 'rb'))
        # 加载词汇表和合并规则
    ...
    def _apply_merges(self, tokens: list[bytes]) -> list[bytes]:
        ...
        while True:
            ...
            # 找所有可合并对及其优先级
            candidate_pairs = [
                (pair, self.merges.index(pair)) for pair in pairs if pair in self.merges
            ]
...
if __name__ == "__main__":
    ...
    DATA_DIR = "/home/lintao/llm_codes/cs336/data"
    vocab_name = "TinyStoriesV2-GPT4-valid-bpe-10000-vocab.pkl"
    merges_name = "TinyStoriesV2-GPT4-valid-bpe-10000-merges.pkl"
    vocab_filepath = os.path.join(DATA_DIR, vocab_name)
    merges_filepath = os.path.join(DATA_DIR, merges_name)
    special_tokens = ["<|endoftext|>"]
    tokenizer = BPE_Tokenizer.from_files(vocab_filepath, merges_filepath, special_tokens)
